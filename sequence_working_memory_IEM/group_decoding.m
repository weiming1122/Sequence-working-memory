function part02_color_Alpha_decoding(subs)
clc;
clear;
close all;
...
svmECOC.alltime = -1000:5:1990; % time points of interest in the analysis
...
for s = 1:nSubs
    ...
    channel = DecodingData.color_list; % tip location of sample teardrop
    svmECOC.posBin = channel'; % add to fm structure so it's saved
    posBin = svmECOC.posBin;
    
%     if sn == 9 
%         ReleventChan = [1:30,63,31:62,64]; % subject09 repair channel 'F8(31)' (ordination change)
%     end
    ...
    % set up time points
    tois = ismember(alltimes,svmECOC.time); nTimes = length(tois);
    ...
    OutputfName = strcat(saveLocation,'/Color_Results_Alphabased_',currentSub,'.mat');
